...
/*
app.get('/syncDatabase', async (rec, res) => {

}

const bodyParser = require('body-parser');
const programer = require('./database/tables/programmer');
...
app.listen(port, () => {
    console.log(`Now listening on port ${port}`);
/*
a.get('/syncDatabase', async (rec, res) => {
    ppconst database = require('./database/db');
...
app.post('/createProgrammer', async (req, res) => {
  try {
    ...
    app.get('/retriveProgrammer', async (req, res) => {
        try {
            const params = req.body;
